    ...
    Public Class Form1
        ...
        End Function
            ...
            For i As Integer = 0 To RowCount - 1
                tbl.Rows.Add(New Object() { String.Format("Name{0}", i), i, 3 - i, Date.Now.AddDays(i), GetBitmap(New Size(30, 30), GetRandomColor(), GetRandomColor(), i.ToString()) })
            Next i
            Return tbl
             End Function
            ...
            Using bmp As New Bitmap(r.Width, r.Height, PixelFormat.Format32bppArgb)
                Using imgGraphics As Graphics = Graphics.FromImage(bmp)
                    Using bufferedGraphics As XtraBufferedGraphics = XtraBufferedGraphicsManager.Current.Allocate(imgGraphics, r)
                        ...
                        Dim [property] As PropertyInfo = GetType(LayoutView).GetProperty("DrawCard", BindingFlags.Instance Or BindingFlags.NonPublic)
                        [property].SetValue(layoutView1, layoutCard, Nothing)
        ...
        Private Sub MoveRow(ByVal sourceRow As Integer, ByVal targetRow As Integer)
            ...
            Dim val1 As Decimal = DirectCast(row1(OrderFieldName), Decimal)
            If row2 Is Nothing Then
                dragRow(OrderFieldName) = val1 + 1
            Else
                Dim val2 As Decimal = DirectCast(row2(OrderFieldName), Decimal)
